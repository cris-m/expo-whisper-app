...
public class ExpoWhisperModule: Module {
    ...
    public func definition() -> ModuleDefinition {
        ...
        AsyncFunction("initContext") { (options: [String: Any]) -> [String: Any] in
            ...
            let useGpu = options["useGpu"] as? Bool ?? true
        ...
        AsyncFunction("startBufferRecording") { (contextId: Int, maxDurationSeconds: Int?) -> String in
            guard let context = self.contexts[contextId] else {
                throw WhisperError.contextNotFound
            }

            do {
                let recordingId = try context.startBufferRecording(maxDurationSeconds: maxDurationSeconds ?? 30)
                return recordingId
            } catch {
